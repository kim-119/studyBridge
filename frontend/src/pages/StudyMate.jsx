...
export default function StudyMate() {
  ...
  const sendMessage = async (e, directMessage = null) => {
    ...
    const visFor = (idx, state) => { if (visualActive()) setProfVisual(roleForAgentIndex(idx), state); };
    const visAll = (state) => { if (visualActive()) setAllProfVisual(state); };
    let sawError = false;
    ...
    // 시각: 질문 전송 → 전체 교수 walking_to_question.
    visAll('walking_to_question');
    ...
    try {
        ...
        try {
          await agentService.streamMessage(userId, agentId, {
            ...
            onAgentStart: (d) => {
              ...
              visFor(d.agentIndex, 'thinking'); // 시각만(append 로직 불변)
            },
            onAgentAnswer: (d) => {
              armWatchdog();
              if (!d) return;
      ...
      // 시각: 스트림 종료 안전망(all_complete/error 누락 시에도 idle 복귀 보장).
      visAll(sawError ? 'error' : 'completed');
