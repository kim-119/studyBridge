...
// (제거됨) PROFESSOR_BUBBLES — 교수 sprite actor 중심 UI로 전환하며 고정 안내 말풍선 삭제.
...
export default function StudyMate() {
  ...
  // 스트림 정상 완료 시각(1분 요약 트리거). 히스토리 로드만으로는 요약이 뜨지 않게 한다.
  const [professorTurnDoneAt, setProfessorTurnDoneAt] = useState(0);
  ...
  // ── 픽셀 교수 시각 상태 setter(답변 append와 무관한 visual only). ───────────────
  //    같은 SSE 이벤트가 중복 수신돼도 상태만 덮어쓴다(배열 push 없음).
  const setProfVisual = React.useCallback((role, state) => {
    if (!role) return;
    setProfessorVisualStates((prev) => (prev[role] === state ? prev : { ...prev, [role]: state }));
  }, []);
  const setAllProfVisual = React.useCallback((state) => {
    setProfessorVisualStates((prev) => (
      (prev.theory === state && prev.book === state && prev.ai === state)
        ? prev
        : { theory: state, book: state, ai: state }
    ));
  }, []);
  ...
  const recapInput = React.useMemo(() => {
    ...
    for (let i = chatHistory.length - 1; i >= 0; i -= 1) {
      ...
      if (!a && m.sender !== 'USER' && !m.isPending && !m.isError) a = m.content;
      if (!q && m.sender === 'USER') q = m.content;
      if (q && a) break;
    }
    return { question: q, answer: a };
  }, [chatHistory]);
  const { recap: professorRecap, dismissRecap: dismissProfessorRecap } = useMinuteRecap({
    question: recapInput.question,
    answer: recapInput.answer,
    trigger: professorTurnDoneAt,
  });
  ...
  const sendMessage = async (e, directMessage = null) => {
    ...
    try {
      ...
      if (STREAMING_ENABLED) {
        ...
        // 이번 턴에 받은 실제 agent_answer 이벤트 수(계약 위반 탐지용 — 프론트는 없는 답변을 만들지 않는다).
        let agentAnswerCount = 0;
        ...
        // ── 교수 상호작용 타임라인 누적(현재 턴). dedupe key(목표 11)로 중복 흡수. ──
        //  백엔드 실제 이벤트 내용만 채운다(목표 12 — 프론트가 내용을 지어내지 않음).
        if (visualActive()) setProfessorInteractionMode(activeLearningMode);
        const interactionSeen = new Set();
        let interactionAcc = [];
        let interactionSeq = 0;
        const pushInteractions = (records) => {
          if (!Array.isArray(records) || !records.length) return;
          let changed = false;
          for (const r of records) {
            if (!r || !r.content || !r.key) continue;
            if (interactionSeen.has(r.key)) continue;
            interactionSeen.add(r.key);
            interactionAcc = [...interactionAcc, r];
            changed = true;
          }
          if (changed && isActiveRequest() && visualActive()) setProfessorInteractions(interactionAcc);
        };
        ...
        try {
          await agentService.streamMessage(userId, agentId, {
            ...
            // (목표 4) professor_motion — 백엔드가 주는 권위 있는 안무. states 를 최우선으로 직접 반영한다.
            //  states 가 없고 phase 만 오면 모드 프로파일에서 phase→states 를 끌어온다.
            onProfessorMotion: (d) => {
              if (!d) return;
            ...
            // synthesis_diff — 최종 답변에 어떤 의견이 반영됐는지(appliedToFinal) 표시.
            ...
            onStageComplete: (d) => {
              ...
              if (d.stage === 2) (d.answers || []).forEach((a) => addInteraction('validation', a));
              else if (d.stage === 3) addInteraction('peer_feedback', { feedbacks: d.feedbacks || [] });
              streamRendered = true;
            ...
            onAllComplete: (d) => {
              ...
              // 계약 검증(텔레메트리만 — 절대 fake agent_answer 를 만들지 않는다).
              //  emitted < selected 면 ai07 가 선택 교수 답변을 충분히 내지 않은 것(예: simulation NPC만).
              const selCount = Number(d?.selectedAgentCount ?? (selectedAgent?.agents?.length || 0));
              const emCount = Number(d?.emittedAgentCount ?? agentAnswerCount);
              if (selCount > 0 && emCount < selCount) {
                console.warn('[StudyMate][contract-violation] agent_answer 부족 — fake 카드 생성 안 함', {
                  mode: activeLearningMode, requestId, selectedAgentCount: selCount, emittedAgentCount: emCount,
                });
              }
  ...
  // ── 픽셀 교수 액션 메뉴 콜백(기존 submit flow와 충돌 없이 입력창 프리필/포커스만). ──────
  ...
  // 답변 비교 — 기존 안전 동작(compare 후속질문) 재사용.
  const handleProfCompare = () => {
    setSelectedProfessorRole(null);
    handleProfessorActionTarget({ professorKey: 'all', actionKey: 'compare', targetKey: 'all' });
  };
  ...
  // 탭이 바뀌면 열린 교수 액션 메뉴 선택을 해제한다(ESC/배경 클릭은 PixelProfessorStage가 처리).
  useEffect(() => { if (!isProfessorTab) setSelectedProfessorRole(null); }, [isProfessorTab]);
              ...
              {isProfessorTab && (
                <div className="professor-discussion-view">
                  {/* 픽셀 교수 stage: classroom 배경 + 교수 sprite 3명(actor) + 액션 메뉴 + 1분 요약.
                      답변 데이터/SSE schema와 무관한 visual consumer(append 로직은 sendMessage에만 존재). */}
                  <PixelProfessorStage
                    ...
                    recap={professorRecap}
                    onRecapDismiss={dismissProfessorRecap}
                    onRefine={handleProfRefine}
                    onAskProfessor={handleProfAskOne}
