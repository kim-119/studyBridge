...
// 답변 톤/성격 → 성격별 professor sprite sheet.
// visual layer 전용 매핑이다. SSE schema / 백엔드 payload는 건드리지 않는다.
export const PERSONALITY_SPRITE_SHEETS = {
  default: '/assets/pixel/professors/personality/professor_default_sheet.png',
  professional: '/assets/pixel/professors/personality/professor_professional_sheet.png',
  friendly: '/assets/pixel/professors/personality/professor_friendly_sheet.png',
  honest: '/assets/pixel/professors/personality/professor_honest_sheet.png',
  unique: '/assets/pixel/professors/personality/professor_unique_sheet.png',
  efficient: '/assets/pixel/professors/personality/professor_efficient_sheet.png',
  cynical: '/assets/pixel/professors/personality/professor_cynical_sheet.png',
};

export const PERSONALITY_LABEL_TO_KEY = {
  default: 'default',
  '기본': 'default',
  '기본값': 'default',
  '차분하게': 'default',
...
export const normalizeSpritePersonalityKey = (raw) => {
  const value = String(raw || '').trim();
  if (!value) return 'default';

  if (PERSONALITY_LABEL_TO_KEY[value]) return PERSONALITY_LABEL_TO_KEY[value];

  const lower = value.toLowerCase();
  return PERSONALITY_LABEL_TO_KEY[lower] || 'default';
};

export const getPersonalitySpriteSheet = (raw, fallbackSheet = PERSONALITY_SPRITE_SHEETS.default) => {
  const key = normalizeSpritePersonalityKey(raw);
  return PERSONALITY_SPRITE_SHEETS[key] || fallbackSheet;
};

export const getProfessorSpriteSheetForAgent = (agent, fallbackSheet) => {
  const personality =
    agent?.personality ||
    agent?.tone ||
  ...
  return getPersonalitySpriteSheet(personality, fallbackSheet);
};
