...
export default function WeeklySchedule({ userId, onTodoCountChange }) {
  ...
  useEffect(() => {
    onTodoCountChange?.(Array.isArray(todos) ? todos.length : 0);
  ...
  const isDateInTodoRange = (dateValue, todo) => {
    if (!dateValue || !todo.startDate) return false;
  ...
  const filteredTodos = Array.isArray(todos)
    ? todos.filter((todo) => isDateInTodoRange(selectedDate, todo))
    : [];
  ...
  return (
    <>
        ...
        <div className="glass-panel calendar-container animate-fade-in">
          <FullCalendar
            ...
            eventContent={(arg) => {
              ...
              return (
                <div style={{ width: '100%', textDecoration: isCompleted ? 'line-through' : 'none', color: isCompleted ? '#9CA3AF' : 'inherit', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', display: 'block' }}>
                  {arg.event.title}
                </div>
              );
            }}
        ...
        <div className="glass-panel todo-section animate-fade-in">
          ...
          {selectedDate && (
            <div style={{ marginBottom: '12px', paddingBottom: '12px', borderBottom: '1px solid var(--color-border)' }}>
              <label style={{ fontSize: '13px', color: 'var(--color-text-muted)', display: 'block', marginBottom: '4px' }}>종료 날짜:</label>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
            <input className="input-field" value={todoText} onChange={(e) => setTodoText(e.target.value)} placeholder="할 일을 입력하세요" disabled={!selectedDate || !userId} />
      ...
      {isEventModalOpen && selectedEvent && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }} onClick={() => setIsEventModalOpen(false)}>
          <div className="glass-panel animate-fade-in" style={{ width: '90%', maxWidth: '400px', padding: '30px' }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ margin: '0 0 16px 0', color: 'var(--color-primary)', borderBottom: '1px solid var(--color-border)', paddingBottom: '12px' }}>Todo 상세</h3>
            <div style={{ display: 'grid', gap: '12px', fontSize: '0.95rem', color: 'var(--color-text-main)' }}>
              <div><span style={{ fontWeight: 600, color: 'var(--color-text-muted)', display: 'inline-block', width: '80px', verticalAlign: 'top' }}>할 일:</span><span style={{ display: 'inline-block', width: 'calc(100% - 85px)', wordBreak: 'break-word', whiteSpace: 'pre-wrap' }}>{selectedEvent.title}</span></div>
              <div><span style={{ fontWeight: 600, color: 'var(--color-text-muted)', display: 'inline-block', width: '80px' }}>시작 날짜:</span><span>{selectedEvent.startStr.split('T')[0]}</span></div>
              <div><span style={{ fontWeight: 600, color: 'var(--color-text-muted)', display: 'inline-block', width: '80px' }}>종료 날짜:</span><span>{(() => { const endDate = new Date(selectedEvent.endStr || selectedEvent.startStr); if (selectedEvent.endStr) endDate.setDate(endDate.getDate() - 1); return endDate.toISOString().split('T')[0]; })()}</span></div>
              <div><span style={{ fontWeight: 600, color: 'var(--color-text-muted)', display: 'inline-block', width: '80px' }}>상태:</span><span style={{ color: selectedEvent.extendedProps.completed ? 'var(--color-text-muted)' : 'var(--color-primary)', fontWeight: 600 }}>{selectedEvent.extendedProps.completed ? '완료' : '미완료'}</span></div>
            </div>
            <div style={{ marginTop: '24px', textAlign: 'right' }}><button className="btn-primary" style={{ width: 'auto', padding: '8px 24px' }} onClick={() => setIsEventModalOpen(false)}>닫기</button></div>
          </div>
        </div>
      )}
    </>
