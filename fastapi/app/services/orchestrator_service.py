...
def _mode_role_directive(mode: str, position: int = 0, total: int = 1) -> str:
    ...
    if m in ("debate", "토론", "토론 모드"):
        base = (
            ...
            "- ★[결론 금지] 사용자가 '판정/결론/누가 이겼' 등을 명시적으로 요청하지 않았다면, 승패·최종 결론을 "
            "내지 마라. 토론은 계속 진행 중이다 — 매 턴 새로운 근거·반례·쟁점으로 논쟁을 '이어가라'."
        )
        if total >= 3:
            role = {
            ...
            }.get(position, "너의 역할: '중립 쟁점 제기자'. 결론·판정을 내지 말고, 양측이 아직 다루지 않은 '새로운 쟁점/반례'를 하나 던져 "
                            "\"그렇다면 ~는 어떻게 설명할 건가?\"처럼 토론을 다음 라운드로 이어가라.")
        elif total == 2:
            role = {
                0: "너의 역할: '논제 정의(없을 때만) + 찬성'. 찬성 입장을 근거로 주장하라.",
            }.get(position, "너의 역할: '반대'. \"나는 반대한다. 왜냐하면…\"으로 반박하고, 결론 대신 새 반례로 논쟁을 이어가라.")
        else:
            role = "혼자이니 '논제: ~인가?'(없을 때만) 정의 후, 찬성 근거와 반대 근거를 각각 \"찬성 측: … / 반대 측: …\"로 대비시켜라. 결론은 내지 마라."
        return base + "\n- " + role
    if m in ("socratic", "소크라테스", "소크라테스 모드"):
