...
def summarize_document(
    ...
    """
    system = (
        "너는 학술 문서 요약 전문가다. 오직 제공된 문서 내용과 해당 문서의 전공 도메인 전문 지식에 근거해서만 한국어로 정리한다.\n"
        "문서의 전체 내용을 누락 없이 파악할 수 있도록, 페이지나 단락(주제)별로 나누어 꼼꼼히 요약하라.\n"
        "반드시 아래 지정된 마크다운 헤더(##) 형식을 정확히 사용하여 응답하고, 서론이나 인사말은 절대 출력하지 마라."
    )
    user = (
        f"## 문서 제목\n{document_title}\n\n"
        f"## 문서 내용 (텍스트 추출본)\n{text[:100000]}\n\n"
        "아래 형식을 엄격히 지켜 문서 전체의 흐름이 잘 드러나도록 구체적으로 작성하라:\n"
        "## 문서 개요\n4~6문장으로 문서 전체의 목적, 배경, 핵심 흐름, 학습자가 얻을 내용을 설명한다.\n"
        "## 핵심 키워드\n문서에서 가장 중요한 명사형 키워드 8~15개를 쉼표로 구분한다.\n"
        "## 세부 핵심 내용\n문서의 각 페이지나 큰 단원 단위로 제목을 작성하고, 핵심 내용을 불릿 포인트로 상세히 정리한다.\n"
        "## 보강 설명\n문서에 등장하는 주요 전문 용어나 개념 중 부연 설명이 필요한 것을 3~5개 골라, **반드시 해당 문서의 학술적/실무적 도메인(예: IT, 경영 등)에 맞게** 정의하고 설명한다. (문맥과 무관한 인물, 대중문화, 뉴스 사건은 절대 언급하지 마라.)\n"
        "## 학습 포인트\n3~5개 불릿으로 시험/실습/복습 관점의 포인트를 적는다.\n"
        "## AI 학습 질문\n이 문서를 깊이 이해했는지 점검할 수 있는 질문 3~5개를 불릿으로 적는다."
    )
    ...
    if images:
        ...
        try:
            ...
            # 이미지와 텍스트를 함께 GPT-4o에 전송 (출력이 잘리지 않도록 토큰 대폭 상향)
            raw = chat_sync(system=system, user=user_content, model="gpt-4o", max_tokens=4096, temperature=0.35)
