...
def summarize_document(
    ...
    user = (
        f"## 문서 제목\n{document_title}\n\n"
        f"## 문서 내용 (전체)\n{text[:100000]}\n\n"
        "아래 형식을 지켜 문서 전체의 흐름이 잘 드러나도록 구체적으로 작성하라:\n"
        "## 문서 개요\n4~6문장으로 문서 전체의 목적, 배경, 핵심 흐름, 학습자가 얻을 내용을 설명한다.\n"
        "## 핵심 키워드\n문서에서 가장 중요한 명사형 키워드 8~15개를 쉼표로 구분한다.\n"
        "## 세부 핵심 내용\n문서의 각 페이지나 큰 단원 단위로 제목을 작성하고(예: 'p.1 개요'), 핵심 내용을 불릿 포인트로 상세히 정리한다.\n"
        "## 보강 설명\n문서에 등장하는 주요 전문 용어나 개념 중 부연 설명이 필요한 것을 3~5개 골라, **반드시 해당 문서의 전공/실무 도메인(예: IT, 데이터베이스 등)에 맞게** 학술적으로 정의하고 설명한다. (문맥과 무관한 뉴스 사건, 대중문화, 인물 등은 절대 설명하지 마라.)\n"
