...
public class ChatService {
        ...
        public ChatDTO.MultiChatResponse chatWithRoom(Long userId, Long roomId, ChatDTO.MultiChatRequest request) {
                ...
                // 이전 AI 답변들 조회
                List<Map<String, String>> previousAnswers = chatMessageRepository
                                .findTop10ByAgentChatRoomIdAndSenderOrderByCreatedAtDesc(roomId, "AI")
                                .stream()
                                .map(msg -> Map.of(
                                                "agentName",
                                                msg.getAgent() != null ? msg.getAgent().getName() : "Unknown",
                                                "answer", msg.getContent()))
                                .collect(Collectors.toList());
                java.util.Collections.reverse(previousAnswers);
